...
export default function ServiceCard({ title, description, badges, time, setCursor, index }) {
  
  const [showService, setShowService] = useState(null);

  return (
    <>
      <motion.div layoutId="service-box" className="pb-5 lg:pb-10 text-white relative mb-8 cursor-none" 
      onClick={() => {setShowService(index)}}
      ...
      {showService === index ? (
        <div className="absolute z-20">
          <motion.div
            layoutId="service-box"
            className=" h-52 w-60 rounded-lg bg-red-800"
          />
        </div>
    ) : null}
    </>
